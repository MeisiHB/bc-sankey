...
export function validateData(data) {
  ...
  data.nodes.forEach((n, i) => {
    ...
    if (n.group && !VALID_GROUPS.includes(n.group)) {
      errors.push('nodes[' + i + ']: unknown group "' + n.group + '". Valid: ' + VALID_GROUPS.join(', '));
    }
...
export function buildNodeMap(nodes, links) {
  ...
  Object.values(nodeMap).forEach(n => {
    ...
    if (n.group === 'result' && n.value < 0) {
      n.color = GROUP_COLORS_UNFAV['result'];
    }
  });
  return nodeMap;
}
...
export function calculateKPIs(data) {
  ...
  const totalRevenue = data.links
    .filter(l => l.target === totalNode?.id)
    .reduce((s, l) => s + l.value, 0);
  const result = data.links
    .filter(l => l.target === resultNode?.id)
    .reduce((s, l) => s + l.value, 0);
  const totalCosts = totalRevenue - result;
  const resultPct  = totalRevenue !== 0
    ? Math.round((result / totalRevenue) * 10000) / 100
    : 0;
  return {
    totalRevenue,
    totalCosts,
    ...
    unitLabel: data.unit_label || 'T\u20AC',
  };
}

export function formatValue(value, unitLabel, decimals) {
  if (unitLabel === undefined) unitLabel = 'T\u20AC';
...
export function resolveNodeColor(node) {
  if (node.group === 'result' && node.value < 0) {
    return GROUP_COLORS_UNFAV['result'];
  }
